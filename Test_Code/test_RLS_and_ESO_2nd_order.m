...
%   y(0) = 10, x(0) = 0;
%   da(t)/dt = 0.5*sin(3t) + w1(t),
%   a(0) = 2.5;
%   b(t) = 2;
%   d(t) = 2.5*sin(t) + 4 + w2(t);
...
pole_tuning = 5; k0 = 3*pole_tuning; k1 = 3*pole_tuning^2; k2 = pole_tuning^3;
ESO_haty(1) = 0; ESO_hatx(1) = 0; ESO_hatd(1) = 0;
...
for k = 1:N+1
    ...
    u_pid(k) = ( kp*(yref(k)-y(k)) + kd*(xref(k)-x(k)) )/b_N;
    ...
    %% ESO
    ESO_error(k) = y(k) - ESO_haty(k);
    ESO_hatdy_dt(k) = ESO_hatx(k) + k0*ESO_error(k);
    ESO_hatdx_dt(k) = RLS_hata(k)*(x(k)+randn) + b_N*u_all(k) + ESO_hatd(k) + k1*ESO_error(k);
    ESO_hatdd_dt(k) = k2*ESO_error(k);
    ESO_haty(k+1) = ESO_haty(k) + dt*ESO_hatdy_dt(k);
    ESO_hatx(k+1) = ESO_hatx(k) + dt*ESO_hatdx_dt(k);
    ESO_hatd(k+1) = ESO_hatd(k) + 0*dt*ESO_hatdd_dt(k);
...
figure('name','System')
subplot(2,1,1); 
plot(index_time, yref(index_data), 'r-', 'linewidth', 1); hold on;
plot(index_time, y(index_data), 'b-.', 'linewidth', 1.5); grid on; 
title('System Output'); legend('y_{ref}', 'y')
subplot(2,1,2); 
